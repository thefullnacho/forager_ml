...
from tensorflow.keras.optimizers import AdamW  # For better generalization
...
class_weights = {0: 1.5, 1: 1.0, 2: 1.0, 3: 1.0}  # Boost deadly
...
# --- AUGMENTATION & DATASET (one-hot labels for categorical loss) ---
aug = A.Compose([
...
def preprocess(img_path, label):
    ...
    label = tf.one_hot(label, depth=4)  # To categorical for smoothing
    return img, label
...
model.compile(optimizer=AdamW(learning_rate=1e-4, weight_decay=0.01),  # AdamW for better dec
              loss=tf.keras.losses.CategoricalCrossentropy(label_smoothing=0.1),  # Smoothing for uplift
              metrics=['accuracy'])
...
model.fit(train_ds.repeat(), validation_data=val_ds.repeat(), 
          ...
          class_weight=class_weights,  # Apply to categorical
...
unfreeze_from = int(total_layers * 0.3)  # Top 70% for more capacity
for layer in base.layers[:unfreeze_from]:
    layer.trainable = False

model.compile(optimizer=AdamW(learning_rate=1e-4, weight_decay=0.01),  # Higher start
...
def tta_predict(ds, num_augs=3):
    preds = []
    for _ in range(num_augs):
        aug_ds = ds.map(lambda x, y: (preprocess('', 0)[0], y))  # Dummy img_path, label not used
        logits = model.predict(aug_ds)
        preds.append(tf.nn.softmax(logits).numpy())
    return np.mean(preds, axis=0)

val_preds_prob = tta_predict(val_ds)
val_preds = np.argmax(val_preds_prob, axis=1)
val_labels = np.argmax(np.concatenate([y for _, y in val_ds], axis=0), axis=1)  # Back to sparse for report
